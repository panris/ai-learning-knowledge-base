...
| Flag | Short | Description |
|------|-------|-------------|
| `--model ` | | Set model with alias (`sonnet`, `opus`, `haiku`) or full model ID |
...
| `--permission-mode ` | | Begin in specified permission mode: `default`, `plan`, `acceptEdits`, `bypassPermissions` |
...
| `--system-prompt-file ` | | Load system prompt from file, replacing default (print mode only) |
| `--append-system-prompt ` | | Append custom text to default system prompt |
| `--append-system-prompt-file ` | | Append file contents to default prompt (print mode only) |
...
| `--plugin-dir ` | | Load plugins from directory for this session only (repeatable) |
...
| Flag | Short | Description |
|------|-------|-------------|
| `--max-budget-usd ` | | Maximum dollar amount for API calls before stopping (print mode only) |
...
| Flag | Short | Description |
|------|-------|-------------|
| `--settings ` | | Path to settings JSON file or JSON string to load |
...
These are top-level commands run as `claude `:
