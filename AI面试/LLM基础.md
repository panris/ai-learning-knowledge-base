...
> `T → 0`：贪心解码，`T → ∞`：均匀分布。
> 实际应用：`T=0` 适合代码生成/数学题（需确定性输出），`T=0.7~1.0` 适合日常对话，`T>1.0` 适合创意写作/头脑风暴（需多样性）。
...
> 将 FFN 层替换为多个 Expert 网络，通过门控路由选择激活。如 Mixtral 8x7B（总参数约 46.7B，每次激活约 12.9B）。
...
> - **开放权重**（Llama、Mistral）：权重可下载、可自部署、可微调，但有使用限制条款（Llama 需遵守社区许可），数据隐私好，能力通常略低
> - **闭源**（GPT-4、Claude）：能力最强，但成本高、数据需经过第三方
> 选择：敏感数据/定制化需求 → 开放权重；追求最强能力 → 闭源
> 注意：Llama 等模型开放的是权重而非源代码训练流程，严格来说属于"开放权重"而非"开源"。
...
| 多轮对话指令遵循率低 | System Prompt 强化 + few-shot 示例 + 思维链(CoT)引导 + 工具调用反馈 |
